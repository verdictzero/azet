class_name TerrainDemo5Screen
extends BaseScreen
## Iteration 5 of the terrain demo — Demo 4 + central platform spawn area
## and a HUD compass. The platform sits dead-centre in the spawn zone
## (zone (0, 0)), the player starts standing on top of it, the ground
## underneath fades from solid metal plating to grass via a noise-modulated
## proxy-splatmap layer, large vegetation (trees / bushes / rocks) is
## excluded within R_OCCLUDE of the platform centre, and the resulting
## clearing's outer rim is filled with an unusually thick fern band. A 3D
## compass rendered with its own orthographic SubViewport is locked to the
## lower-right of the main viewport — its rotor counter-rotates the orbit
## camera yaw so the N-marker always points world-north.
##
## Implementation notes:
## - Metal layer extends `terrain_splat_v2.gdshader`. Metal weight texture
##   is baked on the same worker pool as the meadow density image and
##   shares its UV math (`bake_origin` / `texture_world_size`) so CPU
##   bakers and the GPU shader read identical bytes (proxy-splatmap pattern).
## - Platform model + compass model are re-skinned with the same matcap
##   shader the rocks / bushes / ferns / obelisks use (`rock_matcap.gdshader`
##   + `matcap_2.png`) so they read as worked-metal pieces in the same
##   visual key as the obelisks. No PBR, no wind.
## - Compass SubViewport is rendered separately and its texture is composited
##   inside the main viewport (above fog, below the raster/LUT pass) so the
##   compass gets ASCII-dithered with the rest of the world.
##
## (Following text is the unchanged Demo 4 docstring.)
...
const PlatformScene: PackedScene = preload("res://assets/models/platform_0.glb")
const CompassScene: PackedScene = preload("res://assets/models/compass_0.glb")
# Reuse the existing path texture as the metal albedo. (GROUND_PATH_TEX above
# already points at city_metal_plate_2.jpg; this alias makes the spawn-area
# usage explicit and lets us re-tile it independently of the corridor paths.)
...
# Visual + physics scale applied to the platform GLB. 0.75 = 25% smaller.
const PLATFORM_SCALE: float = 0.75
# Sink the platform partially into the ground (Y -= sink_ratio * height) so
# it reads as embedded rather than dropped on top.
...
# Width of the noisy falloff band where metal dissolves into grass.
const R_BREAKUP_BAND: float = 6.0
# "Very large" object spawn occlusion: nothing larger than a fern is allowed
# within this radius of the platform centre. Carves out the central ~28% of
# the spawn chunk's area as the inciting clearing.
const R_OCCLUDE: float = 30.0
# Inner-edge margin for the fern band — ferns start this far past the
# platform edge so the deck has breathing room before the overgrowth begins.
# (The band's outer edge tracks R_OCCLUDE — together they fill the entire
# dead zone with ferns rather than making a thin ring at the perimeter.)
const PLATFORM_FERN_INNER_MARGIN: float = 1.5
# Cluster anchors are scattered uniformly across each chunk; only those that
# land in the annulus contribute. The annulus is now much wider than before
# (platform_edge → R_OCCLUDE) so we crank attempts up to keep density high.
const PLATFORM_FERN_ATTEMPTS_PER_CHUNK: int = 256

# Secondary point of interest — broken arm prop placed halfway between the
# platform (zone centre) and the NE wall corner. Same spawn-mask + fern
# ring pattern as the platform, just smaller scale. Random Y rotation
# (seeded for stability across reloads).
const BrokenArmScene: PackedScene = preload("res://assets/models/broken_arm_0.glb")
const R_BROKEN_ARM_OCCLUDE: float = 12.0
const BROKEN_ARM_FERN_INNER_MARGIN: float = 1.0
const BROKEN_ARM_FERN_ATTEMPTS_PER_CHUNK: int = 96
const BROKEN_ARM_RNG_SEED: int = 0xBADA55
# Compass HUD viewport size in main-viewport pixels. (Main viewport renders at
# half the grid's pixel size, so 96 here = ~192 native pixels after the LUT
# pass upscales 1:1 in the texture rect.)
const COMPASS_VIEWPORT_PX: int = 96
const COMPASS_INSET_PX: int = 8
const COMPASS_ORTHO_SIZE: float = 1.0  # tuned at runtime from the model AABB
# Tilt the compass back away from the player so it reads as a 3D compass
# rather than a flat top-down overlay. 0 rad = flat top-down; PI/2 = edge-on.
const COMPASS_TILT_RAD: float = deg_to_rad(35.0)
# Names that might identify the rotating part of compass_0.glb. Case-insensitive
# substring match; first hit wins. The actual node in compass_0.glb today is
# `compass_moving_-Y_in_blender_is_north`, so "moving" leads. The rest are
# fallbacks for if the asset is re-authored with a different naming
# convention.
...
class BakeJob extends RefCounted:
	var img: Image = null
	# Demo 5: per-chunk metal-plating weight image. Same overhang convention
	# (BAKE_GRID_N²) as `img` so it can ride the same shader UV math.
...
# Demo 5: platform spawn anchor + metal-splat centre.
...
# the metal splat's solid-core radius and the fern band's inner edge so the
# spawn area reads consistently when the platform's size changes.
var _platform_footprint_r: float = 0.0
# Demo 5: secondary prop — a broken arm at the NE midpoint with its own
# spawn mask + fern ring (same pattern as the platform, smaller scale).
var _broken_arm_root: Node3D = null
var _broken_arm_center_xz: Vector2 = Vector2.ZERO
var _broken_arm_footprint_r: float = 0.0
# Demo 5: HUD compass.
...
# Both the platform and the compass now use their authored GLB materials —
# the platform shows its diffuse / metal-rough / normal maps, the compass
# shows its baked baseColorFactor values (gold ring + red north). Both
# require lighting (added in `_build_world` and `_build_compass_viewport`).
...
func _build_world() -> void:
	...
	# Demo 5: the central platform is a StandardMaterial3D PBR mesh (authored
	# in the GLB) and needs lighting to show its diffuse / metal-rough /
	# normal maps. Existing terrain / foliage / rock shaders are all
	# `render_mode unshaded` so they ignore lights and ambient — the new
	# lighting only affects the platform.
	...
	_platform_center_xz = Vector2(zone_centre, zone_centre)
	# Demo 5: spawn the central platform first so we can drop the player on
	# top of it. The platform owns its own concave-mesh collider so the
	# player is held up by `is_on_floor()` after gravity does its job — no
	# edits to `terrain_player.gd`'s GROUND_Y clamp needed.
	var platform_top_y: float = _spawn_platform(scene, Vector2(zone_centre, zone_centre))
	# Capsule extends 0.9 m below the body's origin (radius 0.4, height 1.8,
	# centered on origin). Spawn the player so the capsule's bottom sits a
	# hair above the platform's top — gravity + the collider hold it there.
	_player.position = Vector3(zone_centre, platform_top_y + 1.0, zone_centre)

	# Demo 5: secondary prop — broken arm halfway between the platform
	# (zone centre) and the NE wall corner. NE corner of zone (0,0) is at
	# (2*zone_centre, 0), so midpoint is (1.5*zone_centre, 0.5*zone_centre).
	# Random Y rotation seeded for stability across reloads.
	var broken_arm_rng := RandomNumberGenerator.new()
	broken_arm_rng.seed = BROKEN_ARM_RNG_SEED
	_spawn_broken_arm(scene, Vector2(zone_centre * 1.5, zone_centre * 0.5),
		broken_arm_rng)
	...
	# Demo 5: HUD compass. Lives in its own SubViewport with an orthographic
	# camera so the compass is visually decoupled from the terrain camera /
	# orbit yaw. The TextureRect that displays it is added INSIDE the main
	# viewport (sibling of `_fog_rect`, drawn after it) so the compass sits
	# above the noise/fog overlay and below the raster/dither/LUT pass — same
	# ASCII-dithered look as the rest of the world.
...
func _cleanup() -> void:
	...
	if _compass_rect: _compass_rect.queue_free(); _compass_rect = null
	if _compass_viewport: _compass_viewport.queue_free(); _compass_viewport = null
	_compass_camera = null; _compass_root = null; _compass_rotor = null
	if _platform_root: _platform_root.queue_free(); _platform_root = null
	_platform_footprint_r = 0.0
	if _broken_arm_root: _broken_arm_root.queue_free(); _broken_arm_root = null
	_broken_arm_footprint_r = 0.0
...
func _start_chunk_bake(key: Vector2i) -> void:
	...
	# Capture the platform centre + footprint as locals so the lambda doesn't
	# have to read mutable screen state from the worker thread.
	...
	var task_id: int = WorkerThreadPool.add_task(
		...
		false,
		"terrain-demo-5 chunk bake (%d, %d)" % [key.x, key.y]
	)
	_pending_bakes[key] = { "task_id": task_id, "job": job }


# Per-chunk metal-plating weight image (Demo 5). Mirrors the storage
# convention of `BiomeField.bake_chunk_density_image`: an L8 (n+2)² image
# with 1-texel overhang on every side, sampled by both the GPU shader
# (`metal_weight_tex` in `terrain_splat_v2.gdshader`) and the CPU side at
# the same `bake_origin` / `texture_world_size` UV. Static so worker
# threads can run it without touching engine APIs.
#
...
# the solid core / clear-grass field, so the FBM only modulates the rim.
static func _bake_chunk_metal_image(origin_x: float, origin_z: float,
	...
	for iz in bake_n:
		...
		for ix in bake_n:
			...
			# smoothstep(from, to, x) is 0 below `from` and 1 above `to`,
			# so we use (1 - smoothstep) to get the "1 inside, 0 outside".
...
func _finish_chunk_load(key: Vector2i, density_img: Image, metal_img: Image) -> void:
	...
	# Demo 5: bind the per-chunk metal weight image and the metal albedo.
	# Bake_origin / texture_world_size are shared with the density splat so
	# the metal weight UV math is bit-identical (CPU/GPU read same bytes).
	if metal_img != null:
		var metal_tex := ImageTexture.create_from_image(metal_img)
		ground_mat.set_shader_parameter("metal_weight_tex", metal_tex)
	ground_mat.set_shader_parameter("metal_albedo_tex", PLATFORM_METAL_TEX)
	ground_mat.set_shader_parameter("metal_world_per_tile", PLATFORM_METAL_WORLD_PER_TILE)
...
func _collect_glades(rng: RandomNumberGenerator, key: Vector2i,
	...
	# Demo 5: thick fern annulus immediately outside each prop's occlusion
	# radius. Runs after all other vegetation passes so they're not crowded
	# out by the standard placement, and outside the regular meadow gate so
	# the ring stays unbroken across grass/forest pockets.
	_collect_radial_fern_ring(rng, _platform_center_xz,
		_platform_footprint_r + PLATFORM_FERN_INNER_MARGIN, R_OCCLUDE,
		PLATFORM_FERN_ATTEMPTS_PER_CHUNK, chunk_ox, chunk_oz, fern_out)
	_collect_radial_fern_ring(rng, _broken_arm_center_xz,
		_broken_arm_footprint_r + BROKEN_ARM_FERN_INNER_MARGIN,
		R_BROKEN_ARM_OCCLUDE, BROKEN_ARM_FERN_ATTEMPTS_PER_CHUNK,
		chunk_ox, chunk_oz, fern_out)
...
func _spawn_bush_cluster(rng: RandomNumberGenerator, cx: float, cz: float,
	...
	for i in range(bush_count):
		...
		if _near_obelisk_exclusion(bwx, bwz):
			continue
		if _near_platform_exclusion(bwx, bwz):
			continue
...
# Demo 5: dead-zone around the inciting-incident props (central platform
# AND the broken arm at the NE midpoint). Returns true if (wx, wz) is
# within either object's occlusion radius. Trees, bushes, and rocks call
# this to clear a plaza around each prop. The dedicated fern ring spawners
# intentionally skip it so ferns can fill the perimeter band that the
# larger plants vacated. (Function name is the platform's for legacy
# reasons — every existing call site already wires through here, so adding
# the broken-arm check inline keeps the diff tight.)
func _near_platform_exclusion(wx: float, wz: float) -> bool:
	var dxp: float = wx - _platform_center_xz.x
	var dzp: float = wz - _platform_center_xz.y
	if dxp * dxp + dzp * dzp < R_OCCLUDE * R_OCCLUDE:
		return true
	var dxa: float = wx - _broken_arm_center_xz.x
	var dza: float = wz - _broken_arm_center_xz.y
	if dxa * dxa + dza * dza < R_BROKEN_ARM_OCCLUDE * R_BROKEN_ARM_OCCLUDE:
		return true
	return false


# Demo 5: thick fern annulus around an arbitrary world XZ point. Used by
# both the central platform and the broken arm — same pattern, different
# centre / inner / outer / attempt count. Inner edge sits just past the
# prop's visible perimeter; outer edge IS the prop's exclusion radius, so
# the band visually fills the entire dead zone — large vegetation is
# excluded but ferns aren't, fulfilling "very thick ferns spreading outward
# from the perimeter of the object". Anchor candidates are scattered
# uniformly across the chunk; only those that land in the band contribute.
# Bypasses the meadow gate so the ring stays unbroken across biomes.
func _collect_radial_fern_ring(rng: RandomNumberGenerator,
		center_xz: Vector2, inner: float, outer: float, attempts: int,
		chunk_ox: float, chunk_oz: float,
		fern_out: Array[Dictionary]) -> void:
	# Cheap chunk-level reject: skip if the chunk AABB is entirely outside
	# the annulus. Any chunk whose closest corner is past `outer`, or whose
	# farthest corner is inside `inner`, contributes nothing.
	var clamped_x: float = clampf(center_xz.x, chunk_ox, chunk_ox + CHUNK_SIZE)
	var clamped_z: float = clampf(center_xz.y, chunk_oz, chunk_oz + CHUNK_SIZE)
	var nearest_dx: float = clamped_x - center_xz.x
	var nearest_dz: float = clamped_z - center_xz.y
	if nearest_dx * nearest_dx + nearest_dz * nearest_dz > outer * outer:
		return
	var fdx: float = maxf(absf(chunk_ox - center_xz.x),
		absf((chunk_ox + CHUNK_SIZE) - center_xz.x))
	var fdz: float = maxf(absf(chunk_oz - center_xz.y),
		absf((chunk_oz + CHUNK_SIZE) - center_xz.y))
	if fdx * fdx + fdz * fdz < inner * inner:
		return
	var inner_sq: float = inner * inner
	var outer_sq: float = outer * outer
	for _i in range(attempts):
		var ax: float = chunk_ox + rng.randf() * CHUNK_SIZE
		var az: float = chunk_oz + rng.randf() * CHUNK_SIZE
		var ddx: float = ax - center_xz.x
		var ddz: float = az - center_xz.y
		...
		for _j in range(count):
			...
			var fwx: float = ax + cos(theta) * r
			var fwz: float = az + sin(theta) * r
			# Reject ferns the cluster radius spilled back across the inner
			# edge so the clearing's inside stays clean.
			var rdx: float = fwx - center_xz.x
			var rdz: float = fwz - center_xz.y
			if rdx * rdx + rdz * rdz < inner_sq:
				continue
			var fsc: float = lerp(FERN_SCALE_MIN, FERN_SCALE_MAX, rng.randf())
			var fry: float = rng.randf() * TAU
			var xform := _make_world_xform(Vector2(fwx, fwz), fsc, fry)
...
func _update_camera() -> void:
	...
	# Demo 5: drive the compass rotor from the world camera yaw so the
	# N-marker always points world-north on screen. The static center cone
	# stays fixed (it's not matched by COMPASS_ROTOR_NAME_HINTS).
	#
	# `-yaw` is the correct rotation direction (verified empirically — at
	# rest the rotor tracks the camera the right way). The +PI offset is a
	# 180° rest-pose flip: the GLB's red north arrow happens to point in
	# the OPPOSITE direction from the camera's "up" axis, so without the
	# offset the red marker points south on screen at zero yaw.
	if _compass_rotor != null:
		_compass_rotor.rotation = Vector3(0.0, -_camera_yaw_rad + PI, 0.0)


# ── Demo 5: platform + compass ─────────────────────

# Spawn the central platform with its visible centre at world (xz.x, xz.y).
# Structure:
#   _platform_root  (no scale, owns position incl. recentre offset + sink)
#   ├── visual      (scale = PLATFORM_SCALE, holds the GLB instance)
#   └── body        (no scale, ConcavePolygonShape3D built from pre-scaled
#                    triangles — keeps the static body at unit scale, which
#                    Godot's physics engine prefers)
# The GLB's mesh origin is offset from its visible centre, so we shift
# `_platform_root.position` by the AABB centre to put the actual platform
# silhouette at `xz`. Authored PBR materials from the GLB are NOT
# overridden — diffuse / metallic-roughness / normal textures shade it.
# Returns the world-Y of the platform's top so the caller can drop the
# player onto it.
func _spawn_platform(scene: Node3D, xz: Vector2) -> float:
	...
	# Visual sub-tree: scaled wrapper that holds the GLB. Scale lives on
	# this node, NOT on the body, so the trimesh collider can sit on a
	# unit-scale StaticBody3D.
	var visual := Node3D.new()
	visual.name = "Visual"
	visual.scale = Vector3.ONE * PLATFORM_SCALE
	_platform_root.add_child(visual)
	visual.add_child(PlatformScene.instantiate())
	...
	# `_platform_root` (so `visual.scale` is included). This reflects the
	# 0.75x scaling already.
	...
	# Recentre offset: the GLB's mesh isn't centred on its origin (extents
	# like X∈[-6.8, +24] in model space). To put the visible centre at
	# `xz`, shift the root by the negative of the AABB centre (XZ only).
	var aabb_centre_x: float = aabb.position.x + aabb.size.x * 0.5
	var aabb_centre_z: float = aabb.position.z + aabb.size.z * 0.5
	# Y placement: lift the root so the AABB BOTTOM lands at -sink_y in
	# world coords (i.e. the platform's bottom is buried `sink_y` below the
	# terrain plane Y=0).
	var root_y: float = -aabb.position.y - sink_y
	_platform_root.position = Vector3(xz.x - aabb_centre_x, root_y, xz.y - aabb_centre_z)
	# World-space top of the platform — caller uses this to spawn the player.
	var top_y: float = _platform_root.position.y + aabb.position.y + aabb.size.y
	# Footprint radius: largest horizontal extent / 2. Drives both the metal
	# splat's solid-core radius and the fern band's inner edge.
	_platform_footprint_r = maxf(aabb.size.x, aabb.size.z) * 0.5

	# Tight mesh collider — collect every triangle in `visual`'s subtree,
	# pre-scaled so the body itself can stay unit-scale. ConcavePolygonShape3D
	# is the static-body trimesh shape; works fine for arbitrary geometry.
...
# Spawn the broken arm prop centred on `xz`, with a random Y rotation
# driven by `rng`. Same structural pattern as `_spawn_platform`:
#   _broken_arm_root  (no scale, sits on ground)
#   ├── visual        (carries the random Y rotation, holds the GLB)
#   └── body          (no scale, ConcavePolygonShape3D collider built from
#                      post-rotation triangles)
# The GLB's PBR materials are kept (lit by the scene's directional light).
# The visible centre is recentred to `xz` BEFORE applying rotation, so the
# rotation pivots around the model centre rather than the GLB's authored
# origin.
func _spawn_broken_arm(scene: Node3D, xz: Vector2, rng: RandomNumberGenerator) -> void:
	_broken_arm_root = Node3D.new()
	_broken_arm_root.name = "BrokenArm"
	scene.add_child(_broken_arm_root)
	_broken_arm_center_xz = xz

	# Inner offset wrapper centres the model on the rotation pivot (the
	# `visual` node's origin). After this shift, rotating `visual` rotates
	# the model in place rather than swinging it around the GLB's authored
	# origin.
	var visual := Node3D.new()
	visual.name = "Visual"
	_broken_arm_root.add_child(visual)
	var inst := BrokenArmScene.instantiate()
	if inst is Node3D:
		visual.add_child(inst)
		# Compute the AABB of `inst` in `visual`'s local space, then shift
		# `inst` so its centre lands at visual's origin.
		var inst_aabb: AABB = _gather_local_aabb(inst, visual)
		var inst_centre := inst_aabb.position + inst_aabb.size * 0.5
		(inst as Node3D).position = -inst_centre
	else:
		visual.add_child(inst)
	# Random Y rotation around the now-centred model.
	visual.rotation = Vector3(0.0, rng.randf() * TAU, 0.0)

	# Recompute AABB AFTER recentring + rotation so we get the true
	# world-axis-aligned footprint (used to sit the model on the ground at
	# Y=0 and to size the spawn-mask radius).
	var aabb: AABB = _gather_local_aabb(visual, _broken_arm_root)
	var root_y: float = -aabb.position.y  # bottom on ground plane (Y=0)
	_broken_arm_root.position = Vector3(xz.x, root_y, xz.y)
	_broken_arm_footprint_r = maxf(aabb.size.x, aabb.size.z) * 0.5

	# Tight mesh collider built from post-rotation triangle data so the
	# body stays unit-scale and the shape matches the visible silhouette.
	var triangles := PackedVector3Array()
	_collect_mesh_triangles(visual, _broken_arm_root, triangles)
	if not triangles.is_empty():
		var body := StaticBody3D.new()
		body.name = "BrokenArmCollider"
		var col := CollisionShape3D.new()
		var shape := ConcavePolygonShape3D.new()
		shape.set_faces(triangles)
		col.shape = shape
		body.add_child(col)
		_broken_arm_root.add_child(body)


# Walk the subtree under `node`, accumulate every MeshInstance3D's triangle
# list into `out`, with vertices transformed into `root`'s local space.
# Uses `_transform_to_root` so `visual.scale` (between mi and root) is
# baked into the output — the collider can then sit on a unit-scale body.
...
# Recursively replace `material_override` on every MeshInstance3D in the
# subtree rooted at `node`. Skips nodes that already have an override (so a
# future authored override on the GLB still wins). The same `mat` instance
# is reused for every mesh — every platform/compass surface shades through
# one ShaderMaterial.
static func _apply_matcap_to_meshes(node: Node, mat: ShaderMaterial) -> void:
	if node is MeshInstance3D:
		var mi: MeshInstance3D = node
		mi.material_override = mat
	for c in node.get_children():
		_apply_matcap_to_meshes(c, mat)


# Per-surface matcap override: walks every MeshInstance3D in the subtree
# and creates one ShaderMaterial PER SURFACE, tinting the matcap with the
# authored material's `albedo_color` (= the GLB's `baseColorFactor`). Each
# surface is wired via `set_surface_override_material(idx, mat)` so a single
# mesh can carry multiple distinct tints (e.g. compass_0.glb's Cylinder
# mesh has a gold `compass_ring` surface and a red `compass_north` surface).
# The matcap shader is unlit, so this works regardless of scene lighting.
...
# into the space of `root`. Uses `_transform_to_root` (defined elsewhere in
# this file) instead of `global_transform` so it works before the subtree
# is added to the live scene tree. Returns an empty AABB if no meshes are
# found. The recursive merge runs through an Array holder because GDScript
# passes AABB by value.
...
# Build the compass HUD: separate SubViewport (own ortho camera, transparent
# bg), instantiate `compass_0.glb`, override every mesh material with the
# shared matcap material, locate the named rotor child, and finally place a
# TextureRect that samples the compass viewport in the bottom-right of the
# main viewport (so the LUT/dither pass picks it up alongside the world).
func _build_compass_viewport() -> void:
	...
	# Compass viewport sits at the grid root (sibling of the main viewport) so
	# it renders independently. Only the TextureRect that samples it has to
	# live inside the main viewport.
	grid.add_child(_compass_viewport)

	var compass_env := WorldEnvironment.new()
	var env := Environment.new()
	env.background_mode = Environment.BG_CLEAR_COLOR
	# Compass model is PBR (StandardMaterial3D from the GLB import) with no
	# textures, just baseColorFactor values for `compass_ring` (gold) and
	# `compass_north` (red). Needs ambient + a directional light to read
	# those colors.
	env.ambient_light_source = Environment.AMBIENT_SOURCE_COLOR
	...
	var compass_sun := DirectionalLight3D.new()
	compass_sun.name = "CompassSun"
	# Light from the upper-right relative to the compass camera so the dial
	# face has a clear highlight side and a softer shadow side.
	compass_sun.rotation_degrees = Vector3(-55.0, -25.0, 0.0)
	compass_sun.light_energy = 1.3
	_compass_viewport.add_child(compass_sun)
	...
	# Per-surface matcap shading: every compass surface is rendered with the
	# (unlit) rock_matcap shader, but tinted by the GLB's authored
	# baseColorFactor. So the gold `compass_ring` surfaces (outer ring +
	# central cone) tint the matcap gold, and the red `compass_north`
	...
	# right axis after my basis setup). The rotor still rotates around its
	# local Y axis, which stays the dial-perpendicular axis after the parent
	# tilt — i.e. the needle still spins within the (tilted) dial plane.
	_compass_root.rotation = Vector3(COMPASS_TILT_RAD, 0.0, 0.0)

	# Locate the rotating part by name. First substring hit wins; fall back
	# to logging the available children so we can pin a different name later.
	_compass_rotor = _find_compass_rotor(_compass_root, COMPASS_ROTOR_NAME_HINTS)
	if _compass_rotor == null:
		var names := PackedStringArray()
		_collect_node_names(_compass_root, names)
		push_warning("[compass] no rotor name match in compass_0.glb. Children: %s"
			% [", ".join(names)])

	# Ortho camera, top-down. The compass GLB has its dial in the X-Z plane
	# (Y = dial-perpendicular, see node name `compass_moving_-Y_in_blender_is_north`),
	# so we view along -Y. Camera up is -Z so the "north" direction (-Z in
	# Godot world / glTF coords, post-Blender-axis-conversion) shows as UP
	# on the compass screen at zero rotor rotation.
	var compass_aabb: AABB = _gather_local_aabb(_compass_root, _compass_root)
	var ortho_size: float = COMPASS_ORTHO_SIZE
	...
	# Set basis directly (look_at depends on global_transform which may not
	# be valid before adding to the scene tree). Camera looks along its local
	# -Z; basis columns are (right, up, back). We want:
	#   right = +X, up = -Z, back = +Y (so forward = -Y → look down)
	# This makes world -Z appear as UP on screen, +X as RIGHT.
	...
	var rect_size: Vector2 = Vector2(COMPASS_VIEWPORT_PX, COMPASS_VIEWPORT_PX)
	_compass_rect.size = rect_size
	_compass_rect.position = Vector2(
		vp_size.x - rect_size.x - COMPASS_INSET_PX,
		vp_size.y - rect_size.y - COMPASS_INSET_PX)
	_viewport.add_child(_compass_rect)
...
static func _collect_node_names(node: Node, out: PackedStringArray) -> void:
	out.append(node.name)
	for c in node.get_children():
		_collect_node_names(c, out)
