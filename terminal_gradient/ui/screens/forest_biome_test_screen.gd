...
extends BaseScreen
## FOREST BIOME TEST — formerly Terrain Demo 5, repurposed as the forest
## biome reference scene for the HDRI lighting revamp. Meadow + forest
## chunked terrain via the proxy-splatmap pattern, central platform spawn
## area, HUD compass. The platform sits dead-centre in the spawn zone
...
func _build_world() -> void:
	...
	# panorama drives the sky, ambient term, and reflections; PBR materials
	# (platform, terrain, foliage) all read from the same source.
...
func _build_compass_viewport() -> void:
	...
	# Compass viewport uses the same forest_day HDRI for ambient (sky-sourced)
	# but renders against a transparent background so it composites cleanly
	# over the main scene. No directional light — keeps lighting policy
	# consistent across both viewports.
