...
export class UIManager {
  ...
  drawMainMenu(cols, rows) {
    ...
    const wideWidth = 92;
    const stackedWidth = 46;
    const useWide = cols >= wideWidth + 6;
    const title = useWide ? titleWide : titleStacked;
    const titleWidth = useWide ? wideWidth : stackedWidth;
    const compact = cols < stackedWidth + 6;
    ...
    if (!compact) {
      ...
      const shortTitle = 'T E R M I N A L   G R A D I E N T';
    ...
    if (settlement.tiles) {
      ...
      if (player) {
        ...
        if (px >= 0 && px < worldW && py >= 0 && py < worldH) {
          ...
          r.drawEntityChar(px, py, '@', playerColor);
        }
      }
  ...
  drawDebugMenu(debug, player, timeSystem, weatherSystem, gameState, turnCount) {
    ...
    if (tab === 0) {
      ...
      entries = [
        ...
        { type: 'toggle', label: 'Walk Really Really Fast', key: 'walkReallyReallyFast', value: debug.walkReallyReallyFast },
        { type: 'sep' },
  ...
  getDebugEntries(debug, timeSystem, weatherSystem, renderer) {
    ...
    if (tab === 0) {
      return [
        ...
        { type: 'toggle', key: 'walkReallyReallyFast' },
