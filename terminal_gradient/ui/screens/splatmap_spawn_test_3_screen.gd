...
## alignment (CPU and GPU read the same baked ID texture) but visible
## texel-grid stairstepping (NEAREST sampling). Test 3 keeps Test 2's
## CPU↔GPU read-the-same-bytes invariant while baking *continuous-valued*
## weight fields instead of a discrete ID. The shader bilinear-samples
## those weights and runs argmax per fragment — bilinear interpolation
## of continuous scalars makes the argmax flip-line a sub-pixel-sharp
## curve, so boundaries are smooth. The spawn loop performs the same
## bilinear-then-argmax on the same baked bytes, so spawns still land
## exactly on their colour by construction.
##
## Per chunk:
## - Bake two textures storing 5 raw `_fbm3` weights per texel:
##   `weights_a` (RGBA8) packs w0..w3, `weights_b` (L8) holds w4.
## - Bind both to a per-chunk `ShaderMaterial.duplicate()` with
##   `filter_linear` so bilinear interpolation is automatic on the GPU.
## - Spawn loop samples both Images via `_sample_baked_id_bilinear`,
##   which mirrors GLSL's `texture()` + `LINEAR` + `CLAMP_TO_EDGE` and
##   runs argmax over the 5 blended weights.
## - Warp from Test 2 (`_warp_xz`) carries over verbatim — gives organic
##   boundary *shape*, while bilinear sampling gives boundary *smoothness*.
##   Both compose cleanly because the warp lives entirely at bake time.
## - Bake runs on `WorkerThreadPool` (same threading scaffold as Test 2),
##   so chunk loading never drops the main thread's frame rate.
##
## Sits alongside Tests 1 and 2 (does not replace them) so all three
## architectures remain available for direct A/B/C visual comparison.
...
# Bake one texel of overhang on every side of the chunk so the GPU's
# `filter_linear` can blend ACROSS chunk seams. Without this, each chunk's
# bilinear filter clamps to its own edge texels and the smoothing stops
# half a texel from the boundary — visible as a thin seam between chunks.
# With the overhang, both adjacent chunks bake the same world XZ at their
# shared overhang positions, so the bilinear blend evaluates to the same
# value on either side of the seam → continuous, fully smooth.
# (N+2)² total texels, ~3 % more bake cost than the no-overhang form.
const BAKE_GRID_N: int = SPLAT_GRID_N + 2
...
const GroundShader: Shader = preload("res://assets/shaders/splat_test_3_ground.gdshader")
...
# its `material_override`, populated with that chunk's `weights_a`,
# `weights_b`, and `chunk_origin`. Per-chunk material instances are
# required because a `sampler2D` can't ride MMI INSTANCE_CUSTOM.
...
#                               weights_a: Image, weights_b: Image }
var _chunks_state: Dictionary = {}

# In-flight chunk bakes running on `WorkerThreadPool` worker threads. The
# worker thread runs `_bake_chunk_weights` (pure CPU; touches no engine
# APIs) and writes the two result images into the shared `BakeJob`
# holder. The main thread polls each tick, finalises completed bakes,
# and never blocks waiting.
# key: Vector2i -> { task_id: int, job: BakeJob }
var _pending_bakes: Dictionary = {}


# Holder for a per-chunk async bake. A worker thread populates `img_a`
# (RGBA8 holding w0..w3) and `img_b` (L8 holding w4); main thread reads
# both once `WorkerThreadPool.is_task_completed` returns true. RefCounted
# so the lambda's captured reference keeps the holder alive until both
# the worker AND main thread have released it.
...
func _update_chunks() -> void:
	...
	for key: Vector2i in _pending_bakes:
		...
		if WorkerThreadPool.is_task_completed(entry.task_id):
			...
			if desired.has(key) and entry.job.img_a != null and entry.job.img_b != null:
				_finish_chunk_load(key, entry.job.img_a, entry.job.img_b)
...
func _start_chunk_bake(key: Vector2i) -> void:
	...
	# Worker thread runs only the pure-CPU bake of both weight images.
	# ImageTexture creation, ShaderMaterial.duplicate, MultiMesh population
	# — anything touching Godot's rendering APIs — must stay on the main
	# thread.
	var task_id: int = WorkerThreadPool.add_task(
		func() -> void:
			var imgs: Array = _bake_chunk_weights(job.origin_x, job.origin_z)
			job.img_a = imgs[0]
			job.img_b = imgs[1],
		false,
		"splat-test-3 chunk bake (%d, %d)" % [key.x, key.y]
	)
	_pending_bakes[key] = { "task_id": task_id, "job": job }


# Runs on the main thread once a worker bake has produced both weight
# images. Builds two ImageTextures, wires them onto a per-chunk
# duplicate of `_ground_material`, then runs the synchronous body of
# the original `_load_chunk` to spawn cuboids and shadows.
func _finish_chunk_load(key: Vector2i, weights_a_img: Image, weights_b_img: Image) -> void:
	...
	# Worker thread already populated both weight images; main thread does
	# the texture upload + per-chunk material wiring.
	var weights_a_tex := ImageTexture.create_from_image(weights_a_img)
	var weights_b_tex := ImageTexture.create_from_image(weights_b_img)

	# Per-chunk material: duplicate the template, attach this chunk's
	# pair of weight textures. A `sampler2D` can't ride MMI INSTANCE_CUSTOM,
	# so material-per-chunk is the only option.
	# `bake_origin` is the world-XZ position of the texture's MIN corner —
	# i.e., the chunk's origin shifted half a texel to the left/up so the
	# overhang baked at index 0 lines up. `texture_world_size` is the world
	# extent the texture covers along each axis: (N+2) × step.
	var step: float = CHUNK_SIZE / float(SPLAT_GRID_N)
	var ground_mat: ShaderMaterial = _ground_material.duplicate()
	ground_mat.set_shader_parameter("weights_a", weights_a_tex)
	ground_mat.set_shader_parameter("weights_b", weights_b_tex)
	...
	for i in CANDIDATES_PER_CHUNK:
		...
		# Zone classification gates which prefab type spawns here. The two
		# weight images we sample are the SAME bytes the ground shader will
		# read at the same (wx, wz), and we apply the same bilinear-blend +
		# argmax operation, so by construction the prefab type matches the
		# tile colour beneath it.
		var id: int = _sample_baked_id_bilinear(weights_a_img, weights_b_img, lx, lz)
		...
		if INTENSIVE_LOG:
			...
			# `_sample_baked_id_bilinear`'s self-consistency would surface
			# here. By construction it must agree with `id`.
			var ground_id: int = _sample_baked_id_bilinear(weights_a_img, weights_b_img, lx, lz)
			...
			# Pixel-space sample position: this is the (px, py) the bilinear
			# blend resolves around. Matches the math in
			# `_sample_baked_id_bilinear` — useful for hunting future drift.
			var px: float = lx / CHUNK_SIZE * float(SPLAT_GRID_N) - 0.5
			var py: float = lz / CHUNK_SIZE * float(SPLAT_GRID_N) - 0.5
			print("[splat-test-3] chunk=(%d,%d) i=%03d wx=%.3f wz=%.3f px=(%.2f,%.2f) prefab=%s id=%d rgb=(%.2f,%.2f,%.2f) ground_id=%d ground_rgb=(%.2f,%.2f,%.2f) %s" % [
				key.x, key.y, i, wx, wz, px, py,
	...
	_chunks_state[key] = {
		...
		# Stashed for offline diagnostics — save_png either weight image to
		# eyeball whether the bake matches the rendered ground.
		"weights_a": weights_a_img,
		"weights_b": weights_b_img,
	}
...
# Bake the per-chunk pair of weight images.
# `weights_a` (RGBA8) packs the first four `_fbm3` weights in RGBA.
# `weights_b` (L8) carries the fifth weight in R.
# Bytes are stored linearly (no sRGB), so reading back via `get_pixel`
# returns the raw normalised weight in [0, 1] without gamma conversion.
#
# Texel layout (with 1-texel overhang on every side):
# - Texture is BAKE_GRID_N × BAKE_GRID_N = (SPLAT_GRID_N + 2)².
# - Texel `i` (i = 0..SPLAT_GRID_N+1) at chunk-local lx = `(i - 0.5) × step`.
#   - Texel 0 sits half a step OUTSIDE the chunk's left edge — overlapping
#     into the LEFT neighbour's interior.
#   - Texel SPLAT_GRID_N+1 sits half a step past the right edge —
#     overlapping into the RIGHT neighbour's interior.
# - Adjacent chunks bake the same world XZ at their shared overhang
#   positions, so the GPU's bilinear filter can blend across chunk seams
#   continuously (no half-texel-strip of reduced smoothing at edges).
#
# `_warp_xz(wx, wz)` runs before classification so the warp's organic
# boundary shape is encoded directly into the baked weight fields.
static func _bake_chunk_weights(origin_x: float, origin_z: float) -> Array:
	var data_a := PackedByteArray()
	data_a.resize(BAKE_GRID_N * BAKE_GRID_N * 4)
	var data_b := PackedByteArray()
	data_b.resize(BAKE_GRID_N * BAKE_GRID_N)
	var step: float = CHUNK_SIZE / float(SPLAT_GRID_N)
	for iz in BAKE_GRID_N:
		# (iz - 0.5) × step covers chunk-local z ∈ [-step/2, CHUNK_SIZE + step/2].
		var wz: float = origin_z + (float(iz) - 0.5) * step
		var row: int = iz * BAKE_GRID_N
		for ix in BAKE_GRID_N:
			var wx: float = origin_x + (float(ix) - 0.5) * step
			var warped: Vector2 = _warp_xz(wx, wz)
			var w: PackedFloat32Array = SplatTestZone.zone_weights_at(warped.x, warped.y)
			var off: int = (row + ix) * 4
			data_a[off + 0] = clampi(int(w[0] * 255.0 + 0.5), 0, 255)
			data_a[off + 1] = clampi(int(w[1] * 255.0 + 0.5), 0, 255)
			data_a[off + 2] = clampi(int(w[2] * 255.0 + 0.5), 0, 255)
			data_a[off + 3] = clampi(int(w[3] * 255.0 + 0.5), 0, 255)
			data_b[row + ix] = clampi(int(w[4] * 255.0 + 0.5), 0, 255)
	var img_a := Image.create_from_data(BAKE_GRID_N, BAKE_GRID_N, false, Image.FORMAT_RGBA8, data_a)
	var img_b := Image.create_from_data(BAKE_GRID_N, BAKE_GRID_N, false, Image.FORMAT_L8, data_b)
	return [img_a, img_b]


# CPU equivalent of the ground shader's bilinear `texture()` calls plus
# argmax over the 5 weight values. Accounts for the 1-texel overhang
# baked into the texture (see `_bake_chunk_weights`):
# - Texel index = chunk_local / step + 0.5 (the +0.5 shifts because
#   texel 0 sits at lx = -step/2, not lx = +step/2).
# - Pixel position used by the bilinear blend is `texel_index`, since
#   that's the texel-centre convention.
# Bit-identical to the GPU shader's `texture(weights, uv)` with the same
# `bake_origin` + `texture_world_size` uniforms.
static func _sample_baked_id_bilinear(
	img_a: Image, img_b: Image, local_x: float, local_z: float
) -> int:
	var step: float = CHUNK_SIZE / float(SPLAT_GRID_N)
	# Pixel position (texel-centre convention). The +0.5 shifts to
	# account for the half-step overhang baked at index 0.
	var px: float = local_x / step + 0.5
	var py: float = local_z / step + 0.5
	# Clamp to valid range to mirror CLAMP_TO_EDGE.
	px = clampf(px, 0.0, float(BAKE_GRID_N - 1))
	py = clampf(py, 0.0, float(BAKE_GRID_N - 1))
	var ix0: int = int(floor(px))
	var iy0: int = int(floor(py))
	var ix1: int = mini(ix0 + 1, BAKE_GRID_N - 1)
	var iy1: int = mini(iy0 + 1, BAKE_GRID_N - 1)
	var fx: float = px - float(ix0)
	var fy: float = py - float(iy0)

	# Fetch 4 RGBA neighbours from weights_a + 4 R neighbours from weights_b.
	var aa: Color = img_a.get_pixel(ix0, iy0)
	var ab: Color = img_a.get_pixel(ix1, iy0)
	var ac: Color = img_a.get_pixel(ix0, iy1)
	var ad: Color = img_a.get_pixel(ix1, iy1)
	var ba: float = img_b.get_pixel(ix0, iy0).r
	var bb: float = img_b.get_pixel(ix1, iy0).r
	var bc: float = img_b.get_pixel(ix0, iy1).r
	var bd: float = img_b.get_pixel(ix1, iy1).r

	# Bilinear-blend each weight channel. Channel order MUST match
	# `splat_test_3_ground.gdshader`'s argmax: RGBA = w0..w3, b.r = w4.
	var w0: float = lerp(lerp(aa.r, ab.r, fx), lerp(ac.r, ad.r, fx), fy)
	var w1: float = lerp(lerp(aa.g, ab.g, fx), lerp(ac.g, ad.g, fx), fy)
	var w2: float = lerp(lerp(aa.b, ab.b, fx), lerp(ac.b, ad.b, fx), fy)
	var w3: float = lerp(lerp(aa.a, ab.a, fx), lerp(ac.a, ad.a, fx), fy)
	var w4: float = lerp(lerp(ba,   bb,   fx), lerp(bc,   bd,   fx), fy)

	# Argmax — same order as the GPU shader.
	var best: int = 0
	var best_w: float = w0
